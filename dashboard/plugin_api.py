...
async def status() -> Dict[str, Any]:
    ...
    return {
        ...
        "key_preview": _redacted_key(api_key),
        "base_url": _base_url(api_key),
        "model": os.environ.get("XIAOMI_TTS_MODEL") or DEFAULT_MODEL,
        "voice": os.environ.get("XIAOMI_TTS_VOICE") or DEFAULT_VOICE,
        "style_prompt": os.environ.get("XIAOMI_TTS_STYLE_PROMPT", ""),
        "models": MODELS,
        "voices": VOICES,
    }
...
async def synthesize(body: Dict[str, Any]) -> Response:
    ...
    voice = str(body.get("voice") or os.environ.get("XIAOMI_TTS_VOICE") or DEFAULT_VOICE)
    ...
    return Response(
        ...
        headers={"Content-Disposition": 'inline; filename="mimo-preview.wav"'},
    )
