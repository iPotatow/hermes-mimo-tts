(function () {
  ...
  function Field(props) {
    return h("label", { className: "mimo-field" },
      h("span", { className: "mimo-label" }, props.label),
      props.children,
      props.hint ? h("span", { className: "mimo-hint" }, props.hint) : null
    );
  }

  function MetaItem(props) {
    return h("div", { className: "mimo-meta-item" },
      h("span", { className: "mimo-meta-label" }, props.label),
      h("strong", { className: "mimo-meta-value" }, props.value)
    );
  }
  ...
  function StatusPanel({ status, loading, onRefresh }) {
    ...
    return h(Card, { className: "mimo-status-card" },
      h(CardHeader, { className: "mimo-card-header" },
        h("div", null,
      ...
      h(CardContent, null,
        h("div", { className: "mimo-meta-grid" },
          h(MetaItem, { label: "API Key", value: configured ? status.key_preview : "未设置" }),
          h(MetaItem, { label: "默认模型", value: status ? status.model : "—" }),
          h(MetaItem, { label: "默认声音", value: status ? status.voice : "—" }),
          h(MetaItem, { label: "输出格式", value: "WAV" })
        ),
        h("div", { className: "mimo-endpoint" },
          h("span", { className: "mimo-meta-label" }, "API 端点"),
  ...
  function Studio({ status }) {
    ...
    useEffect(function () {
      if (!status) return;
      setModel(status.model || "mimo-v2.5-tts");
      setVoice(status.voice || "mimo_default");
      setStyle(status.style_prompt || "");
    }, [status]);
    ...
    return h(Card, { className: "mimo-studio-card" },
      h(CardHeader, null,
        h(CardTitle, null, "语音工作台"),
        h("p", { className: "mimo-subtle" }, "输入文本并试听当前 MiMo 配置")
      ),
  ...
  function MiMoDashboard() {
    ...
    return h("div", { className: "mimo-page" },
      h("header", { className: "mimo-page-header" },
        h("div", null,
          h("div", { className: "mimo-eyebrow" }, "XIAOMI MIMO · SPEECH SYNTHESIS"),
          h("h1", null, "MiMo TTS 控制台"),
